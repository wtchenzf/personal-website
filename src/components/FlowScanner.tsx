...
const FLOW_DATA: Record<string, { ohlc: (OHLCBar & { volume: number })[]; chips: ChipBar[] }> = (() => {
  ...
  // 玩股網實測：04/30 收 4,135 ▲130(+3.25%)，成交量 2,862 張
  // 04/07 低點約 2,490（清明後跳空 -26%），04/24 高點約 4,200
  // volAnchor=1.5 → 04/30 模擬成交量 ≈ 2,850 張 ✓
  d['3661'] = buildFlowData(
    [[0,3350],[3,3060],[5,2820],[8,2590],[9,2490],[11,2730],[13,3090],[16,3500],[19,3860],[22,4160],[23,4200],[24,4090],[26,4135]],
    1.5, 9, 500, 0.030, 3661
  );
  // 3017 奇鋐 — 外資+投信連買11日，液冷散熱龍頭
  // 04/07 低點約 1,960（-26%），04/30 收 3,120
  // volAnchor=12 → 04/30 ≈ 20,000 張（奇鋐高 beta 高換手）
  d['3017'] = buildFlowData(
    [[0,2650],[3,2430],[5,2210],[8,2030],[9,1960],[11,2180],[13,2440],[16,2680],[19,2850],[22,2990],[24,3060],[26,3120]],
    12, 9, 700, 0.024, 3017
  );
  // 2330 台積電 — 外資連買10日，半導體製造龍頭
  // 04/07 低點約 1,660（-16%），04/30 收 2,135 ▼2.06%（勞動節前獲利了結）
  // volAnchor=18 → 04/30 ≈ 30,000 張（台積電日均量）
  d['2330'] = buildFlowData(
    [[0,1980],[3,1900],[5,1800],[8,1710],[9,1660],[11,1750],[13,1840],[16,1930],[19,1990],[22,2070],[24,2180],[26,2135]],
    18, 9, 2000, 0.014, 2330,
    { 26: -1840 }   // 04/30 ▼2.06%：勞動節前獲利了結，法人賣超
  );
  // 6669 緯穎 — 外資連買9日，AI 伺服器 ODM
  // 04/07 低點約 1,380（-25%），04/30 收 2,180
  // volAnchor=5 → 04/30 ≈ 8,600 張
  d['6669'] = buildFlowData(
    [[0,1840],[3,1670],[5,1530],[8,1420],[9,1380],[11,1520],[13,1700],[16,1910],[19,2040],[22,2120],[24,2150],[26,2180]],
    5, 9, 500, 0.026, 6669
  );
  // 5274 信驊 — 外資+投信連買8-9日，BMC晶片龍頭
  // 04/07 低點約 1,100（-25%），04/30 收 1,895
  // volAnchor=2 → 04/30 ≈ 3,700 張（小型股）
  d['5274'] = buildFlowData(
    [[0,1460],[3,1330],[5,1210],[8,1130],[9,1100],[11,1210],[13,1360],[16,1570],[19,1700],[22,1810],[24,1860],[26,1895]],
    2, 9, 200, 0.028, 5274
  );
  // 2382 廣達 — 外資連買8日，AI 伺服器 ODM 最大
  // 04/07 低點約 248（-19%），04/30 收 346 ▼0.57%
  // volAnchor=38 → 04/30 ≈ 60,000 張（廣達高流通）
  d['2382'] = buildFlowData(
    [[0,308],[3,290],[5,272],[8,255],[9,248],[11,263],[13,280],[16,302],[19,320],[22,336],[24,344],[26,346]],
    38, 9, 3000, 0.018, 2382,
    { 26: -520 }    // 04/30 ▼0.57%：短線獲利了結，法人小幅賣超
  );
  // 6442 光聖 — 投信連買10日，矽光子龍頭
  // 04/07 低點約 1,040（-29%），04/30 收 2,160（大漲 +5.37%）
  // volAnchor=2 → 04/30 ≈ 4,300 張（中小型爆量）
  d['6442'] = buildFlowData(
    [[0,1460],[3,1290],[5,1150],[8,1070],[9,1040],[11,1170],[13,1390],[16,1660],[19,1890],[22,2060],[24,2100],[26,2160]],
    2, 9, 300, 0.038, 6442
  );
  // 3222 健策 — 投信連買8日，液冷散熱精密機構
  // 04/07 低點約 516（-24%），04/30 收 898
  // volAnchor=6 → 04/30 ≈ 10,800 張
  d['3222'] = buildFlowData(
    [[0,680],[3,628],[5,576],[8,534],[9,516],[11,556],[13,616],[16,704],[19,775],[22,840],[24,870],[26,898]],
    6, 9, 450, 0.028, 3222
  );
  // 3037 欣興 — 外資連買7日，ABF 載板龍頭
  // 04/07 低點約 158（-17%），04/30 收 219
  // volAnchor=30 → 04/30 ≈ 50,000 張（欣興高流通）
  d['3037'] = buildFlowData(
    [[0,190],[3,181],[5,172],[8,162],[9,158],[11,163],[13,173],[16,185],[19,198],[22,209],[24,215],[26,219]],
    30, 9, 3500, 0.020, 3037
  );
  // 8996 高力 — 投信連買7日，液冷冷排
  // 04/07 低點約 80（-23%），04/30 收 142
  // volAnchor=7 → 04/30 ≈ 11,800 張
  d['8996'] = buildFlowData(
    [[0,104],[3,96],[5,88],[8,82],[9,80],[11,88],[13,97],[16,111],[19,124],[22,133],[24,138],[26,142]],
    7, 9, 600, 0.030, 8996
  );
  // 2454 聯發科 — 外資連買5日，IC 設計龍頭
  // 04/07 低點約 1,720（-20%），04/30 收 2,610
  // volAnchor=25 → 04/30 ≈ 41,800 張
  d['2454'] = buildFlowData(
    [[0,2150],[3,2020],[5,1900],[8,1780],[9,1720],[11,1840],[13,1980],[16,2170],[19,2350],[22,2490],[24,2560],[26,2610]],
    25, 9, 2200, 0.020, 2454
  );
  // 3711 日月光投控 — 外資連買5日，先進封裝 OSAT 龍頭
  // 04/07 低點約 130（-18%），04/30 收 175 ▼0.57%
  // volAnchor=33 → 04/30 ≈ 52,000 張
  d['3711'] = buildFlowData(
    [[0,158],[3,149],[5,141],[8,133],[9,130],[11,136],[13,144],[16,153],[19,161],[22,168],[24,173],[26,175]],
...
const SMART_MONEY: SmartMoneyStock[] = [
  {
    ...
    price: 4135,  changePct: +3.25,
    foreignDays: 14, trustDays: 9,  dealerDays: 3,
    netBuyK: 28450, volRatio: 2.1, pricePct: 28.4,
    signal: 96,
    tags: ['外資連買14日', '投信連買9日', 'ASIC定製', '法人重押'],
    note: '為 NVIDIA 等大廠設計 AI 推論 ASIC，訂單能見度至 2027H1，外資視為 AI ASIC 首選標的持續加碼。清明節後低點 2,490 迅速回補，多頭格局確立。',
  },
  {
