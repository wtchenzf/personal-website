...
const FLOW_DATA: Record<string, { ohlc: (OHLCBar & { volume: number })[]; chips: ChipBar[] }> = (() => {
  ...
  // 04/07 低點約 516（-24%），04/30 收 898，05/08 收 950 ▲30(+3.26%)
  // volAnchor=6 → 04/30 ≈ 10,800 張
  d['3222'] = buildFlowData(
    [[0,680],[3,628],[5,576],[8,534],[9,516],[11,556],[13,616],[16,704],[19,775],[22,840],[24,870],[26,898],[28,924],[31,950]],
    6, 9, 450, 0.028, 3222
  );
...
const SMART_MONEY: SmartMoneyStock[] = [
  ...
  },
  {
    ...
    price: 950,   changePct: +3.26,
    foreignDays: 11, trustDays: 13, dealerDays: 5,
    netBuyK: 8300,  volRatio: 2.0, pricePct: 84.1,
    signal: 80,
    tags: ['投信連買13日', '外資連買11日', '冷板液冷', '跟進奇鋐'],
    note: '液冷散熱冷板製造，受惠奇鋐帶動整個液冷供應鏈。投信連買 13 日，外資近期大幅加碼，05/08 收 950，股本小、籌碼快速集中。',
  },
  {
