...
const FLOW_DATA: Record<string, { ohlc: (OHLCBar & { volume: number })[]; chips: ChipBar[] }> = (() => {
  ...
  // 04/07 低點約 1,980（-24%），04/30 收 3,450（+2.42%），05/08 收 3,650（+2.60%）
  // volAnchor=3 → 04/30 ≈ 5,000 張（高價小型股）
  d['3222'] = buildFlowData(
    [[0,2610],[3,2410],[5,2210],[8,2050],[9,1980],[11,2135],[13,2365],[16,2700],[19,2980],[22,3230],[24,3340],[26,3450],[28,3540],[31,3650]],
    3, 9, 450, 0.028, 3222
  );
...
const SMART_MONEY: SmartMoneyStock[] = [
  ...
  },
  {
    ...
    price: 3650,  changePct: +2.60,
    foreignDays: 11, trustDays: 13, dealerDays: 5,
    netBuyK: 8300,  volRatio: 2.0, pricePct: 84.3,
    signal: 80,
    tags: ['投信連買13日', '外資連買11日', '冷板液冷', '跟進奇鋐'],
    note: '液冷散熱冷板製造，受惠奇鋐帶動整個液冷供應鏈。投信連買 13 日，外資近期大幅加碼，05/08 收 3,650，股本小、籌碼快速集中。',
  },
  {
