...
// idx 對照（33日）: 0=03/24 1=03/25 2=03/26 3=03/27 4=03/30 5=03/31
...
const FLOW_DATA: Record<string, { ohlc: (OHLCBar & { volume: number })[]; chips: ChipBar[] }> = (() => {
  ...
  const b3661: B[] = [
    ...
    {o:5400,h:5530,l:5360,c:5480},
  ];
  // idx 31=05/11 32=05/12 33=05/13 34=05/14 35=05/15 (玩股網實測 mainForce)
  d['3661'] = buildFromReal(b3661, 1.5, 8, 500, 3661, { 31: 409, 32: 77, 33: -755, 34: 256, 35: 250 });
  ...
  const b3017: B[] = [
    ...
    {o:2545,h:2580,l:2475,c:2510},
  ];
  d['3017'] = buildFromReal(b3017, 12, 8, 700, 3017, { 31: 415, 32: -346, 33: 1415, 34: -907, 35: -662 });
  ...
  const b2330: B[] = [
    ...
    {o:2245,h:2290,l:2235,c:2270},
  ];
  d['2330'] = buildFromReal(b2330, 18, 8, 2000, 2330, { 25: -1840, 31: -17876, 32: -1089, 33: -6864, 34: 5937, 35: -86 }); // 04/30 法人賣超；05/11-15 玩股網實測
  ...
  const b6669: B[] = [
    ...
    {o:5360,h:5480,l:5310,c:5430},
  ];
  d['6669'] = buildFromReal(b6669, 5, 8, 500, 6669, { 31: 25, 32: -310, 33: -273, 34: -37, 35: 403 });
  ...
  const b3653: B[] = [
    ...
    {o:3970,h:4010,l:3820,c:3850},
  ];
  d['3653'] = buildFromReal(b3653, 3, 8, 450, 3653, { 30: -750, 31: 449, 32: -122, 33: 61, 34: -209, 35: 49 }); // 05/08 大賣超；05/11-15 玩股網實測
  ...
  const b3037: B[] = [
    ...
    {o:865,h:888,l:858,c:875},
  ];
  d['3037'] = buildFromReal(b3037, 30, 8, 3500, 3037, { 31: 4043, 32: 3425, 33: 2288, 34: 204, 35: -8207 }); // 05/11-14 大量買超後 05/15 法人大賣
  ...
  const b2454: B[] = [
    ...
    {o:3910,h:4020,l:3890,c:3975},
  ];
  d['2454'] = buildFromReal(b2454, 25, 8, 2200, 2454, { 31: -352, 32: -1828, 33: -820, 34: -423, 35: -13 }); // 05/11-15 玩股網實測（外資全週賣超）
  ...
  const b3711: B[] = [
    ...
    {o:540,h:558,l:534,c:548},
  ];
  d['3711'] = buildFromReal(b3711, 33, 8, 3200, 3711, { 25: -380, 31: 2333, 32: 2796, 33: -1583, 34: -1549, 35: 1527 }); // 04/30 小幅減碼；05/11-15 玩股網實測

  // ── 5274 信驊 ── TWSE 無資料，使用合理模擬（BMC晶片，參照3017走勢）
  d['5274'] = buildFlowData(
    [[0,1780],[3,1700],[5,1640],[7,1600],[8,1610],[11,1760],[16,2000],[21,2380],[22,2280],[25,2150],[26,2260],[30,1950],[31,2060],[32,2100]],
    2, 8, 200, 0.028, 5274,
    { 31: 55, 32: 20, 33: -7, 34: 31, 35: -8 }  // 05/11-15 玩股網實測
  );
  // ── 6442 光聖 ── OTC，使用合理模擬（矽光子，高 beta）
  d['6442'] = buildFlowData(
    [[0,1520],[3,1420],[5,1310],[7,1280],[8,1290],[11,1470],[16,1850],[21,2250],[22,2200],[25,2080],[26,2200],[30,2280],[31,2550],[32,2620]],
    2, 8, 300, 0.038, 6442,
    { 31: -152, 32: -1040, 33: -146, 34: 168, 35: 329 }  // 05/11-15 玩股網實測
  );
  // ── 8996 高力 ── OTC，使用合理模擬（液冷冷排，參照同族群）
  d['8996'] = buildFlowData(
    [[0,120],[3,112],[5,103],[7,100],[8,101],[11,116],[16,148],[21,180],[22,175],[25,167],[26,178],[30,178],[31,194],[32,200]],
    7, 8, 600, 0.030, 8996,
    { 31: -120, 32: -165, 33: -231, 34: 87, 35: 33 }  // 05/11-15 玩股網實測（外資全週賣，投信買）
  );
...
const SMART_MONEY: SmartMoneyStock[] = [
  {
    ...
    price: 5580,  changePct: +1.09,
    foreignDays: 20, trustDays: 15, dealerDays: 6,
    netBuyK: 34200, volRatio: 2.1, pricePct: 71.9,
    signal: 97,
    tags: ['外資連買20日', '投信連買15日', 'ASIC定製', '法人重押'],
    note: '為 NVIDIA 等大廠設計 AI 推論 ASIC，訂單能見度至 2027H1，外資視為 AI ASIC 首選標的持續加碼。05/15 週線收高 +1.09% 收 5,580，AI ASIC 籌碼持續堆疊，多頭格局確立。',
  },
  {
    code: '3017', name: '奇鋐',      sector: '液冷散熱',
    price: 2555,  changePct: +1.39,
    foreignDays: 3, trustDays: 17, dealerDays: 5,
    netBuyK: 12800, volRatio: 1.8, pricePct: 66.3,
    signal: 82,
    tags: ['投信連買17日', '外資本週賣超', '液冷龍頭', '量增價漲'],
    note: 'AI 資料中心液冷散熱龍頭，Google、AWS 大單在手。投信連買 17 日，本週外資轉賣（05/11 -239、05/12 -1080、05/14 -1172、05/15 -611），僅 05/13 小幅轉買 +63。投信獨撐，籌碼移轉中，05/15 收 2,555，液冷族群中期多頭格局仍在。',
  },
  {
    code: '2330', name: '台積電',    sector: '半導體製造',
    price: 2265,  changePct: -0.22,
    foreignDays: 2, trustDays: 10, dealerDays: 0,
    netBuyK: 12100, volRatio: 1.4, pricePct: 35.8,
    signal: 91,
    tags: ['投信連買10日', 'CoWoS 滿載', 'N2 順利量產', '指標龍頭'],
    note: '外資本週大幅賣超（05/11 -1.77萬張），投信持續逆勢買進，累積超過 2.8 萬張，顯示內資認為低估。05/14 外資終轉買 +601 張，05/15 外資再小幅賣 -3398 但投信仍買 +3386，合計近乎打平。收盤 2,265，短線外資觀望，投信堅守。',
  },
  {
    code: '6669', name: '緯穎',      sector: 'AI 伺服器',
    price: 5650,  changePct: +1.62,
    foreignDays: 15, trustDays: 12, dealerDays: 4,
    netBuyK: 15200, volRatio: 1.6, pricePct: 64.5,
    signal: 89,
    tags: ['外資連買14日', 'AI伺服器ODM', 'GB200機架優先供應商'],
    note: '微軟、META AI 伺服器 ODM 主力，GB200 機架優先供應商，外資持續 14 日加碼，05/15 週五收高 +1.62% 收 5,650，連續兩日創波段新高，視為 AI 基礎建設直接受益股，法人長期看好。',
  },
  {
    code: '5274', name: '信驊',      sector: 'AI ASIC',
    price: 2200,  changePct: +1.62,
    foreignDays: 15, trustDays: 14, dealerDays: 7,
    netBuyK: 7200,  volRatio: 2.3, pricePct: 81.4,
    signal: 88,
    tags: ['外資連買14日', '投信連買13日', 'BMC龍頭', '量爆突破'],
    note: '全球 BMC 晶片龍頭，AI 伺服器每台必備，營收創歷史新高。外資+投信雙雙連買逾 13 日，05/15 延續上漲 +1.62% 收 2,200，再創波段新高，小型股籌碼堆疊效果強，多頭走勢確立。',
  },
  {
    code: '2382', name: '廣達',      sector: 'AI 伺服器',
    price: 360,   changePct: +0.84,
    foreignDays: 14, trustDays: 8,  dealerDays: 0,
    netBuyK: 52000, volRatio: 1.3, pricePct: 43.9,
    signal: 83,
    tags: ['外資連買13日', 'AI伺服器ODM最大', 'GB系列大單'],
    note: 'NVIDIA GB200/GB300 最大 ODM，外資連 13 日加碼，Q1 AI 伺服器營收季增 48%，法說會展望樂觀。05/15 上漲 +0.84% 收 360，突破近期整理區，長線多頭結構完整，外資持續加碼。',
  },
  {
    code: '6442', name: '光聖',      sector: '矽光子',
    price: 2815,  changePct: +3.12,
    foreignDays: 13, trustDays: 16, dealerDays: 9,
    netBuyK: 5200,  volRatio: 3.4, pricePct: 123.1,
    signal: 83,
    tags: ['投信連買15日', '外資連買12日', '矽光子龍頭', '爆量突破'],
    note: '矽光子 / 光電整合元件唯一台廠，與 Intel、Broadcom 合作量產中。投信率先大量佈局，已連買 15 日，05/15 再漲 +3.12% 收 2,815，籌碼集中度達 18%，持續創波段新高，動能強勁。',
  },
  {
    code: '3653', name: '健策',      sector: '液冷散熱',
    price: 4080,  changePct: +2.26,
    foreignDays: 12, trustDays: 14, dealerDays: 6,
    netBuyK: 9200,  volRatio: 2.5, pricePct: 14.7,
    signal: 76,
    tags: ['投信連買14日', '外資連買12日', '冷板液冷', '強彈反攻'],
    note: '液冷散熱冷板製造，受惠奇鋐帶動整個液冷供應鏈。05/11 強彈 +10% 後，05/15 週線強彈收紅 +2.26% 收 4,080，屬強勁反彈後正常整理，多頭結構仍完整。',
  },
  {
    code: '3037', name: '欣興',      sector: 'PCB / ABF載板',
    price: 925,   changePct: +1.87,
    foreignDays: 10, trustDays: 12, dealerDays: 0,
    netBuyK: 12900, volRatio: 1.5, pricePct: 50.6,
    signal: 70,
    tags: ['投信積極買', 'ABF載板', 'CoWoS受益', '05/15外資大賣'],
    note: 'ABF 載板龍頭，CoWoS 先進封裝帶動 ABF 需求爆發。本週 05/11-13 三大法人共買超 9,800 張，05/14 投信大買 +3,621 但外資轉賣 -3,446，05/15 外資 -6,133 張大量出脫，投信 -1,146 同步倒貨，主力今日大幅賣出。週線仍在高檔，但籌碼出現警訊。',
  },
  {
    code: '8996', name: '高力',      sector: '液冷散熱',
    price: 222,   changePct: +3.74,
    foreignDays: 0, trustDays: 13, dealerDays: 3,
    netBuyK: 3600,  volRatio: 2.6, pricePct: 107.5,
    signal: 71,
    tags: ['投信連買13日', '外資本週賣超', '冷排龍頭', '投信獨撐'],
    note: '液冷散熱冷排零組件，受惠 AI 伺服器機架液冷趨勢。主動 ETF 00981A / 00992A 均持有。本週外資全週賣超（-144 ~ -682），投信持續買進支撐，法人分歧但投信堅守。05/15 仍漲 +3.74% 收 222，投信撐盤效果明顯，小股本彈性大。',
  },
  {
    code: '2454', name: '聯發科',    sector: 'IC 設計',
    price: 3260,  changePct: -4.26,
    foreignDays: 1, trustDays: 0,  dealerDays: 0,
    netBuyK: 335, volRatio: 0.9, pricePct: 42.3,
    signal: 52,
    tags: ['外資今轉買', 'AI手機', 'Dimensity旗艦', '高檔回調'],
    note: '外資本週持續賣超（05/11-14 合計 -3,057 張），本週五外資終轉買 +335 張，但自營大賣 -348 張，合計幾近平衡。05/04-05 連續漲停後本週高檔整理，05/15 下跌 -4.26% 收 3,260，投信本週無參與，短線需觀察量能是否重返。',
  },
  {
    code: '3711', name: '日月光投控', sector: '先進封裝',
    price: 572,   changePct: +1.24,
    foreignDays: 11, trustDays: 9,  dealerDays: 3,
    netBuyK: 31000, volRatio: 1.3, pricePct: 46.2,
    signal: 67,
    tags: ['外資連買11日', '投信連買9日', '先進封裝', '法人共識買'],
    note: '全球最大 OSAT，SiP 封裝持續受惠 AI 需求。本週外資＋投信雙向積極買進：05/11 合計 +2,333 張、05/12 +2,796 張；05/13-14 法人轉賣，05/15 再度回補 +1,527 張。外資已連買 11 日，投信連買 9 日，法人共識強。05/15 延續 +1.24% 收 572，連續兩日走強，本益比相較同業偏低，防禦性佳，長線布局價值顯現。',
  },
];
      ...
      {activeTab === 'sector' && (
        <div className="sector-flow-panel animate-fade-in">
          ...
          <p className="flow-source-note">
            ※ 資料來源：TWSE T86 三大法人買賣超統計；05/12 盤後計算。週變化為相較前5個交易日之差值。
          </p>
        </div>
      )}
