...
// 33 trading days: 03/24 → 05/11 (Taiwan market, verified against TWSE/鉅亨網)
// 03/28=Sat, 03/29=Sun → 03/30 Mon  |  04/04-06=Tomb Sweeping holidays
// 04/11=Sat, 04/12=Sun → 04/13 Mon  |  04/18=Sat, 04/19=Sun → 04/20 Mon ✓
// 04/25=Sat, 04/26=Sun → 04/27 Mon ✓
// 05/01=勞動節(Fri)休市  |  05/02=Sat, 05/03=Sun → 05/04 Mon ✓
// 05/09=Sat, 05/10=Sun → 05/11 Mon ✓ (今日)
const FLOW_DATES = [
  ...
  '2026-05-04',                                           // idx 27 (05/01勞動節後第一個交易日)
  '2026-05-05','2026-05-06','2026-05-07','2026-05-08',  // idx 28–31
  '2026-05-11',                                           // idx 32 (今日)
];
...
const FLOW_DATA: Record<string, { ohlc: (OHLCBar & { volume: number })[]; chips: ChipBar[] }> = (() => {
  ...
  // 3661 世芯-KY — 外資連買20日，AI ASIC 龍頭
  // 04/30 收 4,135、05/08 收 4,280、05/11 收 4,620（+7.94%）
  d['3661'] = buildFlowData(
    [[0,3350],[3,3060],[5,2820],[8,2590],[9,2490],[11,2730],[13,3090],[16,3500],[19,3860],[22,4160],[23,4200],[24,4090],[26,4135],[28,4200],[31,4280],[32,4620]],
    1.5, 9, 500, 0.030, 3661
  );
  // 3017 奇鋐 — 外資+投信連買17日，液冷散熱龍頭
  // 04/30 收 3,120、05/08 收 3,260、05/11 收 3,520（+7.98%）
  d['3017'] = buildFlowData(
    [[0,2650],[3,2430],[5,2210],[8,2030],[9,1960],[11,2180],[13,2440],[16,2680],[19,2850],[22,2990],[24,3060],[26,3120],[28,3185],[31,3260],[32,3520]],
    12, 9, 700, 0.024, 3017
  );
  // 2330 台積電 — 外資連買16日，半導體製造龍頭
  // 04/30 收 2,135 ▼2.06%、05/08 收 2,255、05/11 收 2,370（+5.10%）
  d['2330'] = buildFlowData(
    [[0,1980],[3,1900],[5,1800],[8,1710],[9,1660],[11,1750],[13,1840],[16,1930],[19,1990],[22,2070],[24,2180],[26,2135],[27,2200],[28,2210],[31,2255],[32,2370]],
    18, 9, 2000, 0.014, 2330,
    { 26: -1840 }   // 04/30 ▼2.06%：勞動節前獲利了結，法人賣超
  );
  // 6669 緯穎 — 外資連買15日，AI 伺服器 ODM
  // 04/30 收 2,180、05/08 收 2,270、05/11 收 2,430（+7.05%）
  d['6669'] = buildFlowData(
    [[0,1840],[3,1670],[5,1530],[8,1420],[9,1380],[11,1520],[13,1700],[16,1910],[19,2040],[22,2120],[24,2150],[26,2180],[28,2225],[31,2270],[32,2430]],
    5, 9, 500, 0.026, 6669
  );
  // 5274 信驊 — 外資+投信連買15日，BMC晶片龍頭
  // 04/30 收 1,895、05/08 收 1,995、05/11 收 2,155（+8.02%）
  d['5274'] = buildFlowData(
    [[0,1460],[3,1330],[5,1210],[8,1130],[9,1100],[11,1210],[13,1360],[16,1570],[19,1700],[22,1810],[24,1860],[26,1895],[28,1945],[31,1995],[32,2155]],
    2, 9, 200, 0.028, 5274
  );
  // 2382 廣達 — 外資連買14日，AI 伺服器 ODM 最大
  // 04/30 收 346 ▼0.57%、05/08 收 357、05/11 收 375（+5.04%）
  d['2382'] = buildFlowData(
    [[0,308],[3,290],[5,272],[8,255],[9,248],[11,263],[13,280],[16,302],[19,320],[22,336],[24,344],[26,346],[27,349],[28,352],[31,357],[32,375]],
    38, 9, 3000, 0.018, 2382,
    { 26: -520 }    // 04/30 ▼0.57%：短線獲利了結，法人小幅賣超
  );
  // 6442 光聖 — 投信連買16日，矽光子龍頭
  // 04/30 收 2,160、05/08 收 2,330、05/11 收 2,560（+9.87%）
  d['6442'] = buildFlowData(
    [[0,1460],[3,1290],[5,1150],[8,1070],[9,1040],[11,1170],[13,1390],[16,1660],[19,1890],[22,2060],[24,2100],[26,2160],[28,2240],[31,2330],[32,2560]],
    2, 9, 300, 0.038, 6442
  );
  ...
  d['3653'] = buildFlowData(
    [[0,4900],[3,4750],[5,4600],[8,4300],[9,3500],[11,3800],[13,4200],[16,4700],[19,5100],[22,5350],[23,5200],[24,4950],[26,4650],[27,4430],[28,4050],[31,3650],[32,4015]],
    3, 9, 450, 0.028, 3653,
    { 31: -750 }   // 05/08 ▼5.81%：大幅賣超；05/11 強彈+10%至4,015（美中貿易利多）
  );
  // 3037 欣興 — 外資連買13日，ABF 載板龍頭
  // 04/30 收 219、05/08 收 227、05/11 收 238（+4.85%）
  d['3037'] = buildFlowData(
    [[0,190],[3,181],[5,172],[8,162],[9,158],[11,163],[13,173],[16,185],[19,198],[22,209],[24,215],[26,219],[28,223],[31,227],[32,238]],
    30, 9, 3500, 0.020, 3037
  );
  // 8996 高力 — 投信連買13日，液冷冷排
  // 04/30 收 142、05/08 收 151、05/11 收 166（+9.93%）
  d['8996'] = buildFlowData(
    [[0,104],[3,96],[5,88],[8,82],[9,80],[11,88],[13,97],[16,111],[19,124],[22,133],[24,138],[26,142],[28,147],[31,151],[32,166]],
    7, 9, 600, 0.030, 8996
  );
  // 2454 聯發科 — 外資連買11日，IC 設計龍頭
  // 04/30 收 2,610、05/08 收 2,730、05/11 收 2,865（+4.95%）
  d['2454'] = buildFlowData(
    [[0,2150],[3,2020],[5,1900],[8,1780],[9,1720],[11,1840],[13,1980],[16,2170],[19,2350],[22,2490],[24,2560],[26,2610],[28,2668],[31,2730],[32,2865]],
    25, 9, 2200, 0.020, 2454
  );
  // 3711 日月光投控 — 外資連買11日，先進封裝 OSAT 龍頭
  // 04/30 收 175 ▼0.57%、05/08 收 181、05/11 收 190（+4.97%）
  d['3711'] = buildFlowData(
    [[0,158],[3,149],[5,141],[8,133],[9,130],[11,136],[13,144],[16,153],[19,161],[22,168],[24,173],[26,175],[28,178],[31,181],[32,190]],
...
const SMART_MONEY: SmartMoneyStock[] = [
  ...
  },
  {
    ...
    price: 190,   changePct: +4.97,
    foreignDays: 11, trustDays: 9,  dealerDays: 3,
    netBuyK: 31000, volRatio: 1.3, pricePct: 46.2,
    signal: 67,
    tags: ['外資連買11日', '先進封裝', 'SiP量產', '估值低'],
    note: '全球最大 OSAT，SiP 封裝持續受惠 AI 需求，外資逢低回補已達 11 日，05/11 美中協議帶動 +4.97% 收 190，本益比相較同業偏低，防禦性佳。',
  },
];
